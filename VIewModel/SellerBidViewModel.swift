...
class SellerBidViewModel: ObservableObject {
    ...
    func saveBid() {
        ...
        let bidData: [String: Any] = [
            ...
            "endDate": Timestamp(date: endDate),
            "createdAt": Timestamp(date: Date()),
            "status": BidStatus.pending.rawValue
        ]

        Task {
            do {
                // Step 1: Add the bid document
                try await db.collection("bids").addDocument(data: bidData)
                
                // Step 2: Create a notification for the seller
                let notificationData: [String: Any] = [
                    "recipientId": property.ownerId, // The seller is the recipient
                    "type": NotificationType.newBid.rawValue,
                    "title": "New Bid on Your Property!",
                    "message": "\(bidder.fullName) placed a bid of LKR \(bidAmount) on \(property.propertyName).",
                    "isRead": false,
                    "createdAt": Timestamp(date: Date())
                ]

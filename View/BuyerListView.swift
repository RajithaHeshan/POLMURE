...
//                                // This is the key change to enable navigation
//                                NavigationLink(destination: BuyerOfferView(buyer: buyer)) {
            ...
            VStack(spacing: 0) {
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        LazyVStack(spacing: 16) {
                            ForEach(filteredBuyers) { buyer in
                                // The list now uses the new, simplified row view to prevent compiler errors.
                                BuyerListRowView(buyer: buyer)
                            }
                        }
                    }
                    .padding()
...
struct BuyerListRowView: View {
    let buyer: AppUser
    @EnvironmentObject var sessionStore: SessionStore

    @ViewBuilder
    private var destinationView: some View {
        if sessionStore.appUser?.userType == .seller {
            BuyerOfferView(buyer: buyer)
        } else {
